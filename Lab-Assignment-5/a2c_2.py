...
# Refrence https://github.com/rlcode/reinforcement-learning/blob/master/2-cartpole/4-actor-critic/cartpole_a2c.py)
# Import dependencies
...
class A2CAgent:
    ...
    def build_actor(self):
        ...
        actor.add(
            Dense(
                self.action_size, activation="softmax", kernel_initializer="he_uniform"
            )
        )
        actor.summary()
    ...
    def build_critic(self):
        ...
        critic.add(
            Dense(self.value_size, activation="linear", kernel_initializer="he_uniform")
        )
        critic.summary()
        critic.compile(
            loss="mse", optimizer=Adam(lr=self.critic_lr)
        )  # Loss is MSE since we want to give out a value and not a probability.
        return critic
    ...
    def train_model(self, state, action, reward, next_state, done):
        ...
        if done:
            advantages[0][action] = (
                reward - value
            )  # Basically, what do we gain by choosing the action, will it improve or worsen the advantage
            target[0][
                0
            ] = reward  # Fill in the target value to see if we can still improve it in the policy making
        else:
            advantages[0][action] = (
                reward + self.discount_factor * (next_value) - value
            )  # If not yet done, then simply update for the current step.
